...
- **UX polish:** There are some nitpicks I have with the UX, like not having an `exit program` button when in the Fullscreen, immersive exercise viewer, or the color contrast with the immersive view's bottom button bar being poor when the exercise `<canvas />` is a light background color, as well as some mobile layout wonkiness.
